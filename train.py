...
def main(config):
    ...
    if os.path.exists(run_path):
        raise FileExistsError(
            ...
            f"Please change base_run_name in config.yaml."
        )
    ...
    if not os.path.exists(stats_path):
        raise FileNotFoundError(f"Stats file not found: {stats_path}. Run preprocessing.py first.")
    ...
    train_dataset = Sen2FireDataset(
        config['data_path'], config['train_scenes'],
        transform=train_transform, input_strategy=strategy
    )
    val_dataset = Sen2FireDataset(
        config['data_path'], config['val_scenes'],
        transform=val_transform, input_strategy=strategy
    )

    # ✅ REAL DataLoaders (no ellipsis)
    ...
    in_channels = example['input'].shape[0]
